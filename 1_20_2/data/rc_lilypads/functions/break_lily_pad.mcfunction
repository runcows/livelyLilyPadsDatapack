...
execute as @a[advancements={rc_lilypads:left_click_on_lilypad=true}] at @s if entity @s[gamemode=survival] as @e[type=minecraft:marker,sort=nearest,tag=lilyPad] at @s align xyz run summon minecraft:item ~.5 ~ ~.5 {Motion:[0d,0.2d,0d],Item:{id:"minecraft:lily_pad",Count:1b}}
